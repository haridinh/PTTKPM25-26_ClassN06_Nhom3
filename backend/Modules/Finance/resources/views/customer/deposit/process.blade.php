        ...
        <div class="col-md-8 col-lg-6">
            <div class="card p-3 mb-3">
                ...
                <ul class="warning-list">
                    <li class="text-warning">{{ localize('Please only send') }}
                        <strong>{{ $depositData->currency }}</strong>
                        {{ localize('to this deposit address. Sending any other coin or token to this address may result in the loss of your deposit.') }}
                    </li>
                    <li class="text-danger">{{ localize('Coins will be deposited immediately after') }} <font
                            color="#03a9f4">
                    ...
                    @endphp
                    <li class="text-primary">{{ localize('Payment must be sent immediately, within') }}
                        {{ $timeInHour }}
                        {{ $hourText }}. {{ localize('Sending the payment after') }} {{ $timeInHour }}
                        {{ $hourText }} {{ localize('may result in the loss of your deposit') }} .</li>
                </ul>
            </div>
            <div class="row justify-content-center">
                <div class="col-md-9">
                ...
                <div class="col-md-9 text-center">
                    <a href="{{ route('customer.deposit.index') }}"
                        class="btn btn-outline-stake">{{ localize('Back') }}</a>
                    <a href="{{ $depositData->status_url }}" class="btn btn-outline-stake"
                        target="_blank">{{ localize('Txn Status') }}</a>
                    <a href="{{ $depositData->checkout_url }}" class="btn btn-outline-stake"
